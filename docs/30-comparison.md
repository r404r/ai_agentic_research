...
| **治理与沙箱** | **运行时治理**：runtime online/offline、agent capacity、并发槽、串行规则、权限/可见性、可回溯的 issue/comment 活动历史；隔离工作目录由 daemon 准备 | **Policy/Guardrails 拦截** request/response/tool_call/tool_result（ALLOW/ASK/DENY），成本/工具/安全策略；**OS 沙箱**：Linux bwrap、macOS seatbelt、云 sandbox/managed host |
| **协作模式（指人类协作）** | **异步、任务流式**：issue board、评论触发、@mention、autopilot（定时/手动/webhook）、squad 专家路由；以“看板 + 评论”协作 | **同步/实时、会话式**：live session 共享、co-drive、attach、fork、share；以“实时会话 + web UI”协作 |
...
| **目标用户** | 软件团队 / 多角色协作团队 / 想把多 AI agent 纳入正式任务管理、自动化、审计流程的组织 / 已用多种 CLI 且希望纳入正式流程的个体开发者 | 个人开发者、远程协作团队、agent/平台开发者、企业/受控环境用户 |

> **关于「协作模式」轴**：此轴对比的是**人类协作**形态（multica 看板异步 vs omnigent 活会话实时），不是 agent 编排的同步性。omnigent 的 agent 编排层本身也支持**异步** sub-agent（见 [20-omnigent.md](20-omnigent.md) §4 的 inbox / `async_work_complete`），不应被读成“omnigent 只能同步”。

## 2. 差异解读

### 2.1 编排载体不同：任务流 vs 会话流

- **multica** 把 agent 编排建立在**任务管理原语**（issue / comment / squad / autopilot）之上，天然异步、活动可回溯，适合“把 agent 当同事排进看板”。
...
- **multica** 的治理偏**运行时与组织层**：并发、容量、串行、可见性、可回溯的 issue/comment 活动历史、runtime 在线状态。
...
> **综合判断（本研究在综合两份证据时给出，非 Phase 1 原文结论；置信度见 [40-synthesis-and-judgments.md](40-synthesis-and-judgments.md) J1，不确定度为「中」）。**

两者的**主要定位不同**，更多处于 agentic 栈的不同层次、可叠加使用：

- **omnigent** 偏“**单机/会话级的多 harness 运行时**”——让一个人或一个会话能统一驱动并编排多种 agent。
- **multica** 偏“**团队级的任务编排与运行时治理平台**”——让一个组织能把多个 agent 当成可被分配、触发、且活动可回溯的成员。

但二者**并非完全互补、存在重叠**：在**单机 / 单用户的本地多 agent 编排**场景下，omnigent 的会话级编排与 multica 的本地 runtime 执行功能确有交叠，存在竞争关系。因此本研究**不把“互补”当作确定结论**。

一个可能的协同设想是：omnigent 作为某个 multica runtime/provider 之下的执行外壳，或 multica 在团队层调度、omnigent 在会话层运行。**但此协同关系目前无直接代码证据，仅为推断**，详见 [40-synthesis-and-judgments.md](40-synthesis-and-judgments.md) §2。
